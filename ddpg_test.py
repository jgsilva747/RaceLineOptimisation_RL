...
if __name__ ==  '__main__':
    ...
    while max_count < int( 1e2 ):
        ...
        print("Episode: {}  Total Reward: {:0.2f}  Current Distance: {:0.1f}  Max Distance: {:0.1f}  Exploration Factor: {:0.2f}  Mean Actions: {:0.3f} , {:0.3f}".format( episode, total_reward, current_distance, max_distance, exploration_factor, np.mean(agent_action[:,0]), np.mean(agent_action[:,1])))
        agent.update()
        if episode % 100 == 0:
            agent.save()
        episode += 1
